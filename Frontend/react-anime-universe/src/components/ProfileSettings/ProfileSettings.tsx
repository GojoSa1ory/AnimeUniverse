...
function ProfileSettings() {
  ...
  const updateUser = async (e: React.MouseEvent<HTMLButtonElement, MouseEvent> | undefined) => {
    e?.preventDefault()
  ...
  const updatePassword = async (e: React.MouseEvent<HTMLButtonElement, MouseEvent> | undefined) => {
    e?.preventDefault()
    ...
    AuthService.updatePassword(currentUser?.username, user).then(
      data => {
        localStorage.removeItem("token")
        localStorage.setItem("token", data.data.token)
        setUser(data.data)
  ...
  const updateImage = async () => {
    try {
      ...
      await $api.put(`/user/${currentUser?.username}/image`, formData);
      window.location.reload()
  ...
  const deleteUser = async () => {
    AuthService.deleteUser(currentUser?.username)
    .then(() => {
      localStorage.removeItem("token")
      window.location.reload()
    })
